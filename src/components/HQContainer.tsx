...
const HQContainer = ({ comicsList }: hqContainerProps) => {
    return <div style={style  as React.CSSProperties}>
        {comicsList.map(comic => <HQElement key={comic.collectionNumber} asset={`dcgbr${zeroPad(comic.collectionNumber,3)}_br_1.webp`} title={comic.label} status={comic.status}/>)}
    </div>
}
